...
### Parameter Optimization Algorithm: Twiddle (line 211~251)
...
* Dynamically adjusted the car speed based on the cte which adds to consistent driving. By using Throttle variable, I adjusted the car speed to slow down when the error grows, when error is stable in the small range than I make the car speed. (line 131-142)
* Set the range [0.05,0.3] for throttle value (line 144-157) if cte is over 1.5 then set the throttle as 0.05  (line 153-157) and re-initialize P
...
Calculate steering value , where steering value is [-1, 1]. (line 106-112) If the steer_value is out of this range, set the value -1.0 or 1.0
